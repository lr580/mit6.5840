...
可以不必事务ID，幂等可以让requestid做。

##### 分布式事务

> 分布式存数据，故每个 group 做事务的一部分。需要两阶段提交2PC和两阶段锁2PL
>
> 2PL保证可串行化，增长阶段加锁不放锁，缩减阶段放锁不加锁。缺点是级联回滚(A对了B改的数据，B回滚，A也要回滚)，严格两阶段锁S2PL规定事务持有写锁到提交/中止才能释放，防止脏读。强严格SS2PL读写锁都如此。2PL可能会死锁，需要检测机制，且锁竞争降低吞吐。
>
> 2PC，准备阶段协调者(发起事务)给所有参与者请求prepare，参与者锁定资源执行操作但不提交，反馈结果。协调者发现全票通过，发送commit，收到ack，确认完成。如果有不通过，一起回滚。缺点：①阻塞；②单点故障；③没收到commit会导致不一致，可以用3PC等。

客户端需要自增事务id，实现S2PL。在外层 client 做：

- 把 compare/success/fail 按 shard 分组，搞出每个 shard 要做的子事务 compare/success/fail，然后按 shard 顺序排好。

  合理性：同key顺序一致即可，不同key顺序不一样是可以的，因为get/put不会影响其他key(不依赖，相互独立)。

- prepare 阶段，取group不ok就需要重试，否则用 group 的 Clerk 发起 RPC `TxnPrepare`，得到 wrong group / 事务冲突 / 未准备好 / 不OK，都重试，成功就记录结果。此外，记录分布式compare的与结果，看看后面走success还是failure

  > 其中，事务冲突重试，是破坏循环等待来避免死锁。

  如果重试，把不用重试的 prepared 组遍历发送 `TxnAbort` 事务中断，然后sleep100ms重试。

- commit 阶段，取group不ok就返回事务结果 wrong group。否则，发送 `TxnCommit`，有不是 OK 的回答就返回这个 Err，否则保存执行返回结果(按shard顺序即可)，最后把结果和 compare 交付。

  > 返回结果的排列顺序，在分布式事务没有要求，不影响正确性。除非业务有要求(那么具体业务自己去做)。

对内层 `Clerk`，仿制其他方法包装几个函数即可，reply err 的判断丢给 client 来做。

对服务端，做好持久化格式与缓存，包装接口，然后主要功能：

- `doTxnPrepare`。如果 frozen / 无分片数据，是 wrong leader。

  检查当前存储的 key 是否有其他事务加锁(锁的粒度是key)，初始化 key-事务id 的键值对。以及已经 prepare 还没 commit/abort 的事务信息，是事务id-事务(keys, success, fail, compareOK, prepared)的键值对。如果 prepared 自己了，就OK不用重放。注意这两个内容也需要持久化。

  否则开始做，对所有 key 按字典序去重排序，然后遍历锁表，不区分读写一律加排他锁，加失败了就回滚把刚加的锁都扔了，返回事务冲突，否则继续走。

  > 理由是：锁资源按顺序获取，破坏循环等待条件

  然后做 compare，得到结果，构造事务存储到还没 commit/abort，构造RPC返回结果。

- `doTxnCommit`。如果 frozen / 无分片数据，是 wrong leader。

  检查当前事务id是否正在准备，如果查无或不准备，返回事务冲突。然后选择suc/fail，执行并获得全部结果。然后，把所有持有的锁释放。删除该事务，返回结果。

- `doTxnAbort`。如果无分片数据 wrong leader。然后能取到事务，解锁，删除事务，返回。
...
- 2026/1/30 5h 完成Lab5D的选做：事务、分布式事务(2PC, 2PL)